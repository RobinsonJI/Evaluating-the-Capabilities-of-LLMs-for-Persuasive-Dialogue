...
from persuasio.routers.mas_graph_routers import (
    check_whether_models_starts_the_dialogue,
    llm_response_router
)
...
def create_mas_workflow():
    ...
    workflow.add_node("InitialClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="initial_llm_claim",
    ...
    workflow.add_node("SinceClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="since_claim",
            schema=SinceClaimResponses,
            output_key="___Since___")
    )
    workflow.add_node("ClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="claim",
            schema=ClaimResponses,
            output_key="___Claim___")
    )
    workflow.add_node("ClaimNegationGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="claim_negation",
            schema=ClaimNegationResponse,
            output_key="___Claim___")
    )
    workflow.add_node("WhyClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="why_claim",
            schema=WhyClaimResponses,
            output_key="___Why___")
    )
    workflow.add_node("QuestionClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="question_claim",
            schema=QuestionClaimResponses,
            output_key="___Question___")
    )
    workflow.add_node("ConcedeClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="concede_claim",
            schema=ConcedeClaimResponses,
            output_key="___Concede___")
    )
    workflow.add_node("RetractClaimGenerationAgent", lambda state : DialogueMoveGenerator(
        state=state
        ).generate(
            prompt_method_name="retract_claim",
    ...
    workflow.add_node("DisambiguationAgent", lambda state : DisambiguationAgent(
        state = state
    ...
    ===================================================================================================================================================================
    '''
    workflow.add_conditional_edges(START, check_whether_models_starts_the_dialogue)

    workflow.add_edge("UtteranceClassificationAgent", "IdentifyModelsSetOfTypicalResponses")
...
mas_workflow = create_mas_workflow()
mas_graph = compile_sub_graph(workflow=mas_workflow)
draw_graph(graph=mas_graph, graph_name="mas_graph")