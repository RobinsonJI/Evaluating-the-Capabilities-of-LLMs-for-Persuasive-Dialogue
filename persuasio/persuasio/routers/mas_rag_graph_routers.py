...
@log_function
def check_for_vector_based_persona_examples(state : GenerationAgentsState) -> Literal["VectorBasedRAGPersonaGenerationAgent", "UtteranceClassificationAgent"]:

    if len(state["vector_based_persona_string"]) == 0 and (len(state["vector_based_persona_sys_prompt"]) == 0):
        return "VectorBasedRAGPersonaGenerationAgent"
    else:
        # if the vector-based persona has been generated, then the model can generate typical responses
        return "UtteranceClassificationAgent"
    
@log_function
def check_for_model_start_dialogue_after_vector_based_personas(state: GenerationAgentsState) -> Literal["InitialClaimGenerationAgent", "UtteranceClassificationAgent"]:
